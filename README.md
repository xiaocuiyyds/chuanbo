...
87 页管路图/接线图上的信息对纯文本检索是黑箱：设备位号印在位图里，全书文字层中
`GFV`、`GFP` 出现次数都是 0。系统从图纸里取用两样东西——位号和介质管路，前者让用户
输一个阀件位号就能定位到对应的图，后者由 PDF 矢量对象直接读出、精确且零成本。

器件类型和器件级拓扑没有做成，也不进索引。做过的尝试、量出来的结论和未尽事项记在
[docs/diagram-understanding.md](docs/diagram-understanding.md)——那是一个独立方向，
不是本系统的一部分。
